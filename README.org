...
- AUR, Nix, Guix, Gentoo, and Void recipes are local/manual only for now; the GitHub Actions workflow builds/tests the release tarball plus DEB/RPM jobs, not those local/manual package ecosystems.
...
=make test-nix-flake-container= runs =nix flake check= inside a clean =docker.io/nixos/nix= container; set ~NIX_CONTAINER_IMAGE=...~ to try another image. Inside =nix develop=, =DEC_FONTS_ASDF_LOAD_MODE= is set to =asdf= so =scripts/generate-fonts.sh= loads =dec-fonts= through ASDF directly. Outside Nix, the default remains Quicklisp; set =DEC_FONTS_ASDF_LOAD_MODE=asdf= only when dependencies such as =png= and =array-operations= are already visible to ASDF.

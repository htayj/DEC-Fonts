...
- =make test-deb-package=, =make test-rpm-package=, and =make test-arch-package= install the packages in Debian/Fedora/Arch containers and verify the font files plus fontconfig visibility. Set ~CONTAINER_RUNTIME=podman~ if you prefer Podman.
- AUR and Nix packaging are local/manual only for now; the GitHub Actions workflow builds/tests the release tarball plus DEB/RPM jobs, not AUR uploads or Nix builds.

The DEB package installs PSF console fonts under =/usr/share/consolefonts/dec-fonts=. The RPM package installs them under =/usr/lib/kbd/consolefonts/dec-fonts=. The Arch package installs them under =/usr/share/kbd/consolefonts/dec-fonts=. All three packages install BDF/OTB fonts under =/usr/share/fonts/dec-fonts=, data under =/usr/share/dec-fonts=, docs under =/usr/share/doc/dec-fonts=, and enable a package-scoped fontconfig config.

...
usage() {
    cat <<'USAGE'
usage: scripts/stage-linux-package.sh --format deb|rpm|arch|gentoo|void --destdir DIR
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --format)
            [[ $# -ge 2 ]] || fail "--format requires deb, rpm, arch, gentoo, or void"
            format=$2
            shift 2
            ;;
        --format=*)
...
case "$format" in
    deb|rpm|arch|gentoo|void) ;;
    "") fail "--format is required" ;;
    *) fail "--format must be deb, rpm, arch, gentoo, or void: $format" ;;
esac
...
require_file packaging/fontconfig/75-dec-fonts.conf
...
case "$format" in
    deb|gentoo) psf_dest="$destdir/usr/share/consolefonts/dec-fonts" ;;
    rpm) psf_dest="$destdir/usr/lib/kbd/consolefonts/dec-fonts" ;;
    arch|void) psf_dest="$destdir/usr/share/kbd/consolefonts/dec-fonts" ;;
esac
...
install -D -m 0644 packaging/fontconfig/75-dec-fonts.conf "$fontconfig_available"
mkdir -p "$(dirname "$fontconfig_enabled")"
cp -p "$fontconfig_available" "$fontconfig_enabled"
chmod 0644 "$fontconfig_enabled"
